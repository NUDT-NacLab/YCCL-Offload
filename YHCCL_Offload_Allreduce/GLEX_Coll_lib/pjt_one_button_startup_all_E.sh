...
export CPLUS_INCLUDE_PATH=/usr/local/mpi3-static/include:$CPLUS_INCLUDE_PATH
export C_INCLUDE_PATH=/usr/local/mpi3-static/include:$C_INCLUDE_PATH
export LD_LIBRARY_PATH=/usr/local/mpi3-static/lib:$LD_LIBRARY_PATH
...
#
export  procName=alltoall-test-integer
...
export C_INCLUDE_PATH=/usr/local/glex/include/:$C_INCLUDE_PATH
cp /usr/local/glex/include/glex.h ./include/
cp /usr/local/mpich-3.2a-test/lib/libmpicxx.so.0 ./
...
EXCLUDE_NODE_LIST=cn2567,cn2570,cn2027,cn2566
...
case $MODE in
    ...
    7   )
        ...
        echo  echo \"  " `cat ./batch.sh | sed -n '2,2p'`" \" | bash
        yhbatch -p $PARTITION   -N $GLEX_COLL_NODEN --time=0-0:6:20  ./batch-alltoall.sh
        ;;
    8   )
    ...
    11   )
        ...
        wait
        yhbatch -p $PARTITION  -N $GLEX_COLL_NODEN_MAX --time=2-4:20:20  ./batch-allreduce.sh
         ;;
    12   )
