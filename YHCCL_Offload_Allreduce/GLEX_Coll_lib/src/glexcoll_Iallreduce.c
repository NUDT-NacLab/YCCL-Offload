...
int GLEXCOLL_Iallreduce_Tree(void *sendbuf, void *recvbuf, int send_count, enum glex_coll_reduce_op GLEX_OP)
{
	...
	if (_ReduceTreeS[_TreeID].type == LEAF)
	{
		...
		GLEX_Coll_req.flag = GLEX_FLAG_COLL | GLEX_FLAG_COLL_SEQ_START | GLEX_FLAG_NO_CONNECTION |
	...
	if (_ReduceTreeS[_TreeID].type == MID)
	{
		...
		// if(inter_rank != 1)
		{
			...
			GLEX_Coll_req.flag = GLEX_FLAG_COLL | GLEX_FLAG_COLL_SEQ_START | GLEX_FLAG_COLL_SEQ_TAIL |
								 GLEX_FLAG_COLL_CP_SWAP_DATA | GLEX_FLAG_COLL_SWAP | GLEX_FLAG_NO_CONNECTION |
		...
		// {
		// 		glex_coll_compose_mp_set_data(&(_GLEXCOLL.ep_addrs[inter_rank]), 0,
		// 						GLEX_Coll_req.data, &GLEX_Coll_req.len);
		// 		GLEX_Coll_req.rmt_ep_addr.v = _GLEXCOLL.ep_addrs[inter_rank].v; /* one for itself */
		// 		GLEX_Coll_req.flag = GLEX_FLAG_COLL | GLEX_FLAG_COLL_SEQ_START | GLEX_FLAG_COLL_SEQ_TAIL
		...
		for (i = 0; i < _ReduceTreeS[_TreeID].childsN; i++)
		{
			...
			{
				// puts("iph_check");
				GLEX_Coll_req.flag = GLEX_FLAG_COLL | GLEX_FLAG_COLL_REDUCE_RESULT | GLEX_FLAG_COLL_SWAP | GLEX_FLAG_COLL_CP_DATA | GLEX_FLAG_NO_CONNECTION |
									 GLEX_FLAG_COLL_COUNTER | GLEX_FLAG_COLL_CP_SWAP_DATA; // GLEX_FLAG_COLL_SEQ_TAIL
			}
			else
			{
				//_ReduceTreeS[_TreeID].childTypes[i] == LEAF
				GLEX_Coll_req.flag = GLEX_FLAG_COLL | GLEX_FLAG_COLL_REDUCE_RESULT | GLEX_FLAG_COLL_CP_DATA | GLEX_FLAG_NO_CONNECTION |
	...
	if (_ReduceTreeS[_TreeID].type == ROOT)
	{
		...
		// }
		{
			...
			// 	glex_coll_compose_mp_set_data(_ReduceTreeS[_TreeID].childAddrs, _ReduceTreeS[_TreeID].childsN,
			// 								GLEX_Coll_req.data, &GLEX_Coll_req.len);
			// 	GLEX_Coll_req.rmt_ep_addr.v = _GLEXCOLL.ep_addrs[intra_rank].v; /* one for itself */
			// 	GLEX_Coll_req.flag = GLEX_FLAG_COLL | GLEX_FLAG_COLL_SEQ_START | GLEX_FLAG_COLL_SEQ_TAIL | GLEX_FLAG_COLL_REDUCE_RESULT
			// 		| GLEX_FLAG_COLL_CP_SWAP_DATA | GLEX_FLAG_COLL_SWAP |
...
{
	static int glex_mp_recv_count = 0;
	if (inter_procn > 1)
	{
		...
		if (intra_rank == 0)
		{
			...
			// if(0)
			{
				...
				// if(inter_rank == 0)// || inter_rank == 1|| inter_rank == 2
				{
					...
					{
						void *datarecv;
						while (glex_probe_next_mp(_GLEXCOLL.ep, &rmt_ep_addr, &datarecv, &GLEXCOLL_databuf_len) != GLEX_SUCCESS)
							;
						glex_mp_recv_count++;
						glex_coll_decompose_mp_reduce_data(data_loc,
														   _GLEXCOLL._allreduce.count,
														   _GLEXCOLL._allreduce.bit_size,
														   datarecv);
						if (glex_mp_recv_count % 1024 == 0)
						{
							glex_mp_recv_count = 0;
...
int GLEXCOLL_Iallreduce(void *sendbuf, void *recvbuf, int count, MPI_Datatype datatype, MPI_Op op)
{
	if (_GLEXCOLL.inter_procn > 1)
	{
		...
		// void *tmpbuf = (void *)malloc(sizeof(double) * _GLEXCOLL._allreduce.count);
		static double tmpbuf[40];
		...
		if (intra_procn > 1)
			...
			memcpy(tmpbuf, sendbuf, count * 8);
		if (intra_rank == 0)
		{
			// printf("global_rank=%d 数据规约：send data = %f data send = %f\n", global_rank, ((double *)_GLEXCOLL._allreduce.sendbuf)[0], ((double *)tmpbuf)[0]);
